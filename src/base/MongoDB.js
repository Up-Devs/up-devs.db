...
const { DatabaseLatency, MongoData, MathOperator } = require('../util/Constants');
...
class MongoDB extends Base {
    ...
    async set(key, value) {
        ...
        if (!raw) {
            ...
            await data.save()
                .catch(e => {
                    return new Events(Constants.Events.ERROR, e, this);
                });
            return data.data;
        } else {
    ...
    */
    ...
    async startsWith(key, options) {
        ...
        let all = await this.all(options && options.limit);
